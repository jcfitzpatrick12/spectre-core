...
class TestSTFFT:
    ...
    def test_compute_frequencies(self) -> None:
        ...
        assert is_close(
            spectre_core.events.get_frequencies(window_size, sample_rate),
    ...
    def test_num_spectrums(
        ...
        assert expected_num_spectrums == spectre_core.events.get_num_spectrums(
    ...
    def test_stfft(self) -> None:
        ...
        signal = spectre_core.events.get_cosine_signal(
            num_samples, sample_rate, frequency, amplitude, phase
        )
        window = spectre_core.events.get_window(window_type, window_size)
        buffer = spectre_core.events.get_buffer(window_size)

        # Plan, then compute the STFFT.
        fftw_obj = spectre_core.events.get_fftw_obj(buffer)
        dynamic_spectra = spectre_core.events.stfft(
