...
def _short_sleep() -> None:
    sleep(spectre_core.jobs.Duration.ONE_CENTISECOND)
...
def _make_successful_runtime_worker() -> spectre_core.jobs.Worker:
    return spectre_core.jobs.make_worker("successful_runtime_worker", _sleep_forever)


def _make_instantly_failing_runtime_worker() -> spectre_core.jobs.Worker:
...
@pytest.fixture
def successful_runtime_worker() -> spectre_core.jobs.Worker:
...
@pytest.fixture
def instantly_failing_runtime_worker() -> spectre_core.jobs.Worker:
...
@pytest.fixture
def successful_runtime_job() -> spectre_core.jobs.Job:
    ...
    return spectre_core.jobs.Job(workers)


@pytest.fixture
def partially_failing_job() -> spectre_core.jobs.Job:
...
class TestWorker:
    def test_name(self, successful_runtime_worker: spectre_core.jobs.Worker) -> None:
        """Check that the name of the process is as expected."""
        assert successful_runtime_worker.name == "successful_runtime_worker"

    def test_is_alive(
        self, successful_runtime_worker: spectre_core.jobs.Worker
    ) -> None:
    ...
    def test_is_not_alive(
        self, instantly_failing_runtime_worker: spectre_core.jobs.Worker
    ) -> None:
    ...
    def test_kill(self, successful_runtime_worker: spectre_core.jobs.Worker) -> None:
    ...
    def test_restart(self, successful_runtime_worker: spectre_core.jobs.Worker) -> None:
...
class TestJobs:
    ...
    def test_start(self, successful_runtime_job: spectre_core.jobs.Job) -> None:
    ...
    def test_kill(self, successful_runtime_job: spectre_core.jobs.Job) -> None:
    ...
    def test_monitor_successful_job(
        self, successful_runtime_job: spectre_core.jobs.Job
    ) -> None:
        """Check that once the total runtime of a job is complete, the workers are no longer alive."""
        successful_runtime_job.start()
        successful_runtime_job.monitor(spectre_core.jobs.Duration.ONE_CENTISECOND)
    ...
    def test_monitor_failed_job(
        self, partially_failing_job: spectre_core.jobs.Job
    ) -> None:
        ...
        with pytest.raises(RuntimeError):
            partially_failing_job.monitor(spectre_core.jobs.Duration.ONE_SECOND)

        # Check all the workers are not alive.
        assert not partially_failing_job.workers_are_alive

    def test_max_restarts(self, partially_failing_job: spectre_core.jobs.Job) -> None:
        ...
        with pytest.raises(
            ...
            partially_failing_job.monitor(
                spectre_core.jobs.Duration.TEN_SECONDS,
                force_restart=True,
